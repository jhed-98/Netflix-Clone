...
const MovieCard: React.FC<MovieCardProps> = ({ data }) => {
    ...
    const redirectToWatchMovie = useCallback(() => router.push(`/videos/${data.id}`), [router, data.id]);
    const redirectToWatchTv = useCallback(() => router.push(`/tvs/${data.id}`), [router, data.id]);

    return (
        <div className="group bg-zinc-800 col-span relative h-auto lg:h-[12vw] border border-gray-100">
            <div className=''>
                <img onClick={() => {
                    data.media_type === "movie" ? redirectToWatchMovie : redirectToWatchTv
                }} src={`https://image.tmdb.org/t/p/w500${data.backdrop_path}`} alt="Movie" draggable={false}
            ...
            <div className="opacity-0 absolute top-0 transition duration-200 z-10 invisible sm:visible delay-300 w-full scale-0 group-hover:scale-110 group-hover:-translate-y-[6vw] group-hover:translate-x-[2vw] group-hover:opacity-100">
                <img onClick={redirectToWatchMovie} src={`https://image.tmdb.org/t/p/w500${data.backdrop_path}`} alt="Movie" draggable={false}
                ...
                <div className="z-10 bg-zinc-800 p-2 lg:p-4 absolute w-full transition shadow-md rounded-b-md">
                    ...
                    <div className="flex flex-row items-center gap-3">
                        <div onClick={redirectToWatchMovie} className="cursor-pointer w-6 h-6 lg:w-10 lg:h-10 bg-white rounded-full flex justify-center items-center transition hover:bg-neutral-300">
